...
def init():
    ...
    if maxmind_key is None or not maxmind_key.strip():
    ...
    if response.status_code == 200:
        ...
        logging.critical(
            f"downloading asn file error, error code {response.status_code}"
        )
        exit(1)
        ...
        for file_name in file_list:
            ...
            if file_name.startswith(outer_folder + "/"):
                ...
                target_file_path = os.path.join(
                    asn_folder_path, file_name_without_outer
                )
                # 解压文件到目标路径
...
def download_source_repo():
    ...
    if response.status_code == 200:
        ...
        logging.critical(
            f"downloading rule source error, error code {response.status_code}"
        )
        exit(1)
...
class RuleSet(object):
    def __init__(self, domain, domain_keyword, domain_suffix, ip_cidr, process_name):
        ...
        if (
            len(domain) != 0
            or len(domain_keyword) != 0
            or len(domain_suffix) != 0
            or len(ip_cidr) != 0
        ):
            rule = dict()
            if len(domain) != 0:
...
def translate_rule():
    ...
    logging.info("finish translating clash rules")
...
def translate_source_to_target(entry, source_dir, target_dir):
    ...
    with open(readme_file, "w") as readme:
        readme.write(
            f"# {entry}\n\n#### 规则链接\n\n**Github**\nhttps://raw.githubusercontent.com/quanuanc/singbox_ruleset/main/rule/{entry}/{entry}.srs\n\n**CDN**\nhttps://cdn.jsdelivr.net/gh/quanuanc/singbox_ruleset@main/rule/{entry}/{entry}.srs"
        )
...
def translate_extra():
    ...
    for k, v in extra_surge_conf.items():
        ...
        if response.status_code == 200:
            ...
            logging.critical(
                f"downloading {k}.conf error, error code {response.status_code}"
            )
            exit(1)
        ...
        with open(readme_file, "w") as readme:
            readme.write(
                f"# {k}\n\n#### 规则链接\n\n**Github**\nhttps://raw.githubusercontent.com/quanuanc/singbox_ruleset/main/rule/{k}/{k}.srs\n\n**CDN**\nhttps://cdn.jsdelivr.net/gh/quanuanc/singbox_ruleset@main/rule/{k}/{k}.srs"
            )
